...
import _seq
import count
#from chipregions import ChipRegions
...
class Motif:
    ...
    self.symbols (list of string): list of the transcription factors that
...
class MotifList(list):
    ...
    def from_xml_file(self, path):
        ...
        """
        dom = parse(path)
        motifs_xml = dom.getElementsByTagName("motif")
        for m in motifs_xml:
            tmp = Motif.from_xml(m)
            try:
                self.append(tmp)
            except DuplicateMotif: #ignored
               pass
