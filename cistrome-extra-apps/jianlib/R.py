...
def barplot(height,names=None,beside=False,horiz=False,col=None,main=None,xlab=None,\
            ylab=None,xlim=None,ylim=None,border=True,space=None, cex_names=None, cex_axis=None, density=None):
    ...
    if border == True: rscript+='border=TRUE,'
    elif border == False: rscript += 'border=FALSE,'
    elif border == None: rscript += 'border=NA,'
    ...
    if density != None:
        rscript += 'density=%s,' %str(density)
    
    rscript=rscript[:-1]+')\n'
    return rscript

def pie(x,labels=None, main=None, col=None, clockwise=False, border=False, init_angle = None, cex=1, radius = 0.8, density=None):
...
def legend(x,legend,pch,y=None,col=None,bty='o', density=None):
