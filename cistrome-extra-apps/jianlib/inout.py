...
import CistromeAP.jianlib.graphics as graphics
from CistromeAP.jianlib.corelib import *
...
def test_if_valid_BED( bed ):
    """This function tests if a given BED object is valid or not
    ...
    # if bed object is empty.
    if bed.bed == None or bed.bed == {}:
        return 'File is empty!'

    chroms = bed.get_chroms()
    for chrom in chroms:
        for start, end in itertools.izip( bed[chrom]['start'], bed[chrom]['end'] ):
            # check if they are integers 
            if type(start) != int or type(end) != int:
                return '2nd and 3rd columns must be integers!'

            # check start is smaller than end
            if end - start < 0:
                return '3rd column must be larger than 2nd!'
...
def _draw_barplot_w_val(genome,ChIP,pval,names,main=None,xlab=None,ylab=None,horiz=False,upsidedown=False, cex_names = 1, cex_text = 1,density=100):
    ...
    if horiz:
        ...
        xlim=None
        ylim=[0,maxp+marg]
    rscript=R.barplot([genome,ChIP],names=names,beside=True,horiz=horiz,col=["#5FA1C1", "#EB9D86"],\
                    main=main,xlab=xlab,ylab=ylab,xlim=xlim,ylim=ylim, border=False, cex_names=cex_names, density=None)
...
def draw_pie_distribution_of_elements(Genome_EP, ChIP_EP, gprom=(1000, 2000, 3000), gdown=(1000, 2000, 3000), prom=(1000,2000,3000), down=(1000,2000,3000)):
    ...
    gnames += ["Promoter (%d-%d bp)" %(p1, p2) for p1, p2 in itertools.izip(gprom[:-1], gprom[1:])]
    gnames += ["Downstream (<=%d bp)" %gdown[0]]
    gnames += ["Downstream (%d-%d bp)" %(d1, d2) for d1, d2 in itertools.izip(gdown[:-1], gdown[1:])]
    gnames += ["5'UTR","3'UTR", "Coding exon", "Intron", "Distal intergenic"]

    # get the labels (legend) for the pie chart
    names = ["Promoter (<=%d bp)" %prom[0]]
    names += ["Promoter (%d-%d bp)" %(p1, p2) for p1, p2 in itertools.izip(prom[:-1], prom[1:])]
    names += ["Downstream (<=%d bp)" %down[0]]
    names += ["Downstream (%d-%d bp)" %(d1, d2) for d1, d2 in itertools.izip(down[:-1], down[1:])]
    ...
    names_w_percent_x = map(lambda x, y: x + ': ' + y, names, x_percent)
    ...
    y_percent = _percent_str(map(lambda a: 100.0*a, y))
    names_w_percent_y = map(lambda x, y: x + ': ' + y, gnames, y_percent)
    # make x values less than .1% .5% because they are too small to see in the pie chart. But x_percent does not change
    y = map(max, y, [0.01]*len(y))
...
def draw_chrom_barplot(genome_p,ChIP_p,pval,horiz=True,upsidedown=True):
    ...
    rscript+=R.legend(x='right',legend=['Genome','ChIP (p-value)'],pch=15,col=["#5FA1C1", "#EB9D86"],bty='n')
    return rscript

def draw_promoter_barplot(genome_p,ChIP_p,pval,prom):
    
    names = ['<=%d bp' %p for p in prom]
    ...
    rscript+=R.legend(x='topleft',legend=['Genome','ChIP (p-value)'],pch=15,col=["#5FA1C1", "#EB9D86"],bty='n')
    return rscript

def draw_bipromoter_barplot(genome_p,ChIP_p,pval,biprom):
    
    names = ['<=%d bp' %bp for bp in biprom]
...
def draw_histogram_bar(histogram, lim, main ='', xlab = '', linecol='cyan', bgcol='black', cex=1):
    ...
    # when an error happens, just plot a black rectangle.
    try:
        # draw a histogram
        N = 0.8                         # ratio for normalization
...
def draw_multiple_profiles(breaks, prfls, cols=[], main='', xlab='', ylab='', ylim=[], v=None, legends=[]):
    ...
    if cols:
        ...
        rscript +=R.plot(breaks,prfls[0],col='linecols[1]', main=main, xlab=xlab, ylab=ylab, ylim=ylim, lwd=2) 
        for i in range(1, n_prfls):
            rscript += R.lines(breaks, prfls[i], col='linecols[%d]' %(i+1), lwd=2)
        if not legends:
...
def draw_multiple_profiles2(breaks, prfls, cols=[], main='', xlab='', ylab='', ylim=[], v=None, legends=[]):
    """This function does not add the 'black' line"""
...
def draw_profile_plot(breaks,avg_upstream,avg_downstream,metagene_breaks,avg_metagene,metacatexon_breaks,avg_metacatexon,metacatintron_breaks,avg_metacatintron,metagene_breaks_lim=[-1000,1000]):
    ...
    upbreaks = breaks[ start_up : end_up ]
    downbreaks = map( lambda b: b + metagene_breaks[ -1 ], breaks[ start_down:end_down ] )
    
    # sometimes, the last point of the metagene (e.g. 3000bp) is missing with a certain choice of profile resolution
    # in such case, just use the last point of metagene
    if downbreaks[ 0 ] != metagene_breaks[-1]:
        comb_breaks = upbreaks + metagene_breaks[1:] + downbreaks
        comb_metagene = avg_upstream[ start_up:end_up ] + avg_metagene[1:] + avg_downstream[ start_down:end_down ]
    else:
        comb_breaks=upbreaks + metagene_breaks[1:-1] + downbreaks
    ...
    # if failed in getting ylim, just go ahead with default
    try:
...
def draw_CEAS(genome_p,ChIP_p,pval,bg_res=100,chip_res=600,prom=(1000,2000,3000),biprom=(2500,5000),down=(1000,2000,3000),gene_div=(3,5)):
